...
if (isset($_POST['postdata'])) {
	...
	if (empty($file)) {
		$output = '<div class="alert alert-warning" role="alert">Vui lòng chọn một bức ảnh!</div>';
		 $uploadOk = 0;
	} else {
	...
	$toado = htmlspecialchars($_POST['vitri_geolocation']);
	$pub_on = trim(htmlspecialchars($_POST['public_loc']));
	//Add data
	if ($statuss == 1) {
		if (!empty($img_path)) {
			$post->postNew($email, $phuongxa, $quanhuyen, $tinhtp, $loaiNS, $loaiNSO, $tengiong, $gioithieu, $img_path, $time_start, $time_end, $toado, $pub_on, $img_path1, $img_path2, $img_path3, $img_path4);
			$output = '<div class="alert alert-success" role="alert">Bài viết đã đươc đăng và đang chờ duyêt!</div>'; 
		}
...
<div class="panel panel-custom">
  ...
  <div class="panel-body">
	...
	<div class="panel-body">
	 <form id="frm_post" method="POST" action="/dang-bai.html#post" enctype="multipart/form-data">
	  <div id="baoloi"><?php echo $output; ?></div>
		...
		<div class="panel-heading">Chọn vị trí nông sản của bạn</div>
		 <div class="panel-body">
				...
				<div class="input-group">
					...
					<input type="text" class="form-control" id="vitri_geolocation" onclick="getLocation()" name="vitri_geolocation" readonly placeholder="Toạ độ của bạn">
				</div><br/>
				<!--Tuy chon hien thi-->
						<div class="input-group">
		...
		<input type="text" class="form-control" id="tengiong" name="tengiong" value="" placeholder="Ví dụ: Xoài cát Hoà Lộc" required="required">
		</div><br/>
		<!--Mota-->
		 <label>Mô tả:</label>
		   <div class="form-group">
			<textarea class="form-control" id="about_ns" name="about_ns" rows="3" placeholder="Ví dụ: Mô tả về phương pháp trồng hay sản phẩm của bạn, ..."></textarea>
		  </div>
				<!--AnhSP-->
			<!--<div class="input-group">
	...
	$(".custom-file-input").on("change", function() {
	  ...
	  $(this).siblings(".custom-file-label").addClass("selected").html(fileName);
			...
			<!--<div class="text-center"> <img id="img_news" src="https://i.imgur.com/DVqiWKl.png"/></div>
			   <script type="text/javascript">
		...
		<!--Lengiong/Xuonggiong/Thuhoach-->
				<!--Trong-->
				 <label for="sel1">Thời gian trồng:</label>
				 <!-- Datepicker as text field -->         
				  <div class="input-group date" data-date-format="dd.mm.yyyy">
					<input  type="text" id="time_trong" name="time_trong" value="" class="form-control" placeholder="ngày.tháng.năm trồng hoặc xuống giống (Bỏ trống nếu không nhớ)">
				 ...
				 <!-- Datepicker as text field -->         
				  <div class="input-group date" data-date-format="dd.mm.yyyy">
					<input  type="text" id="time_thuhoach" name="time_thuhoach" value="" class="form-control" placeholder="ngày.tháng.năm thu hoạch (Bỏ trống nếu không nhớ)">
					<div class="input-group-addon" >
					  <span class="glyphicon glyphicon-th"></span> Lịch
					</div>
				  </div>
		...
		<!--POST-->	
			<div class="text-right">
			 <input type="submit" class="btn btn-success btn-sm" name="postdata" id="postdata" value="Đăng bài viết"/>
			</div>
				</div>
		</div>
	</form>
...
<!--Vung can le phai-->
	...
	<div class="col-md-4">
		...
		<div class="panel panel-custom">
		  ...
		  <div class="panel-heading" >Đã duyệt gần đây</div>
